...
#Used for importing live data from website
from flask import Flask, request
from selenium import webdriver
from selenium.webdriver.common.by import By
import time

url = "http://192.168.45.40:8080/"
edge_driver_path = r'D:\Code\292\292-Walk-Run-App-main\msedgedriver.exe'
...
def livestream():
    ...
    driver.get(url)
    graph_tab = driver.find_element(By.XPATH, "//ul[@id='viewSelector']/li[contains(text(), 'Absolute')]")
    graph_tab.click()
    data_values = []
    ...
    while time.time() - start_time < 3:
        ...
        span_element = driver.find_element_by_class_name("valueNumber")
        value = span_element.text
        data_values.append([value])

        time.sleep(0.03)

    csv_file_path = "Static/values.csv"

    # Write the array to a CSV file
    with open(csv_file_path, 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(["Acceleration x (m/s^2)"])
        csv_writer.writerow(["Acceleration y (m/s^2)"])
        csv_writer.writerow(["Acceleration z (m/s^2)"])
        csv_writer.writerows(data_values)
    driver.quit()
    df_live = pd.read_csv("Static/values.csv")
    df_live = classify_activity(df_live)
    return df_live
